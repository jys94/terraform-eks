data "aws_eks_cluster_auth" "cluster" { name = aws_eks_cluster.eks_cluster.id }
...
resource "aws_eks_fargate_profile" "fargate_profile_apps" {
  ...
  selector { namespace = kubernetes_namespace_v1.fp_ns_app1.metadata[0].name }
}
